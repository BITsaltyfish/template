...
namespace KD_Tree {
    struct node {
        ...
        // d表示这个点的坐标[0]->x,[1]->y,mx表示这个平面的x的范围，my表示这个平面的y的范围
        inline void push_up() {
            size = 1;
            for(int i = 0; i <= 1; i++) {
                if(ch[i]) {
    ...
    */
    ...
    ll calc_mn(node *o) { //Manhattan距离下进行计算，如果是欧氏距离，则为max(P.x-o->mx[1],0)+max(o->mx[0]-P.x,0)的平方（本处表示到这个子树平面的最短距离）
        ...
        ret += max(abs(o->mx[0] - P.x), abs(o->mx[1] - P.x));
        ret += max(abs(o->my[0] - P.y), abs(o->my[1] - P.y));
        // ret+=1ll*(max(P.x-o->mx[1],0ll)+max(o->mx[0]-P.x,0ll))*(max(P.x-o->mx[1],0ll)+max(o->mx[0]-P.x,0ll));
        // ret+=1ll*(max(P.y-o->my[1],0ll)+max(o->my[0]-P.y,0ll))*(max(P.y-o->my[1],0ll)+max(o->my[0]-P.y,0ll));
        return ret;
    }
    void Query_Max(node *o) { //查询平面中到目标点最远的点
        ans = max(ans, 1ll * (o->d[0] - P.x) * (o->d[0] - P.x) + 1ll * (o->d[1] - P.y) * (o->d[1] - P.y)); //欧几里得距离
        // ans=max(ans,abs(o->d[0]-P.x)+abs(o->d[1]-P.y)); //曼哈顿距离
    ...
    void Query_Min(node *o) { // 查询平面中到目标点最近的点
        if(!o)return;
        ans = min(ans, 1ll * (o->d[0] - P.x) * (o->d[0] - P.x) + 1ll * (o->d[1] - P.y) * (o->d[1] - P.y)); //欧几里得距离
        // ans=max(ans,abs(o->d[0]-P.x)+abs(o->d[1]-P.y)); //曼哈顿距离
    ...
    node *root;
    ll Query(int x, int y, bool f) { // 传入1为查询最远距离，传入0为查询最近距离
        P = Point(x, y);
        if(f) ans = -inf, Query_Max(root);
        else ans = inf, Query_Min(root);
        return ans;
    }
    // 以下三个函数为本人手写，所以可靠性有待验证，不过过过题....
    void debug(node *&a) {
        if(a->ch[0]) debug(a->ch[0]);
        printf("%lld %lld\n", a->d[0], a->d[1]);
        if(a->ch[1]) debug(a->ch[1]);
    }
    void del(node *&a) { //用于清空整颗树
        for(int i = 0; i <= 1; i++) {
            if(a->ch[i]) del(a->ch[i]);
        }
        a = NULL;
    }
    void init() { //用于初始化
...
void solve() {
    ...
    for(int i = 1; i <= n; i++) {
        scanf("%d%d", &x[i], &y[i]);
        KD_Tree::insert(KD_Tree::root, x[i], y[i], 1);
        if(KD_Tree::rebuild_need)KD_Tree::rebuild();
    }
    for(int i = 1; i <= n; i++) {
        ll s = KD_Tree::Query(x[i], y[i], 0) ;
        printf("%lld\n", s);
    }
    KD_Tree::del(KD_Tree:: root);//清空
}
int main() {
    int t;
    scanf("%d", &t);
