...
#define mx 1000010
ll c1[mx],c2[mx];
void modify(int x,int y){for(int i=x;i<mx;i+=i&(-i))c1[i]+=y,c2[i]+=(ll)x*y;}
//差分数组中位置x加上y
void add(int l,int r,int x){modify(l,x);modify(r+1,-x);}//区间加x
ll ask(int x)//查询前缀和
{
    ll ans(0);
...
void modify(int x,int y,ll z)//差分数组中位置(x,y)加上z
{
	for(int i=x;i<mx;i+=i&-i)
		for (int j=y;j<mx;j+=j&-j)
			c1[i][j]+=z,c2[i][j]+=z*x,c3[i][j]+=z*y,c4[i][j]+=z*x*y;
}
