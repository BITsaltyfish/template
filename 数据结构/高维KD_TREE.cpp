...
long long Dis(const Node &a, const Node &b) {
    long long ret = 0;
    for(int i = 0; i < K; i++) ret += (a.x[i] - b.x[i]) * (a.x[i] - b.x[i]);
    return ret;
}
Node p[maxn];
...
int main() {
    ...
    while(scanf("%d%d", &n, &K) != EOF) {
        for(int id = 1; id <= n; id++) {
            for(int i = 0; i < K; i++) scanf("%d", &p[id].x[i]);
            p[id].id = id;
            a[id] = p[id];
        }
        Build(1, n, 0);
        ...
        Node tar;
        while(Q--) {
            for(int i = 0; i < K; i++) scanf("%d", &tar.x[i]);
            scanf("%d", &k);
            ...
            for(int i = tot - 1; i >= 0; i--) {
                for(int j = 0; j < K; j++) printf("%d%c", a[ans[i]].x[j], j == K - 1 ? '\n' : ' ');
            }
        }
    }
    return 0;
