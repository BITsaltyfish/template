...
struct point{
    ...
    point(datatype _x,datatype _y,int _id){x=_x;y=_y;id=_id;}
    point operator +(const point &b)const{return point(x+b.x,y+b.y);}
    point operator -(const point &b)const{return point(x-b.x,y-b.y);}
    point operator *(const datatype &b)const{return point(b*x,b*y);}
    datatype operator *(const point &b)const{return x*b.x+y*b.y;}
    datatype operator ^(const point &b)const{return x*b.y-y*b.x;}
};
