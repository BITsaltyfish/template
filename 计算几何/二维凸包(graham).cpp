...
inline void graham(int n)
{
    ...
    for(int i=1;i<=n;i++)
    {
        while (top>1&&((p[zhan[top]]-p[zhan[top-1]])^(p[i]-p[zhan[top-1]]))<0)top--;//默认凸包边界直线上的点在凸包上，如果不要算，'<'改'<='即可
        zhan[++top]=i;
    }	
    int now=n-1;while (now>=1&&((p[now]-p[1])^(p[zhan[top]]-p[1]))==0)zhan[++top]=now--;//默认凸包边界直线上的点在凸包上，要加这一句
}
inline void work()
{
