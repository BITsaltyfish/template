#define data_type int // 记得改类型！记得改类型！记得改类型！记得改类型！记得改类型！
...
void FFT(data_type *a, data_type *b, int n, int m, data_type *c) {
    // 直接调用，n,m是a,b的长度，下标0开始,c是卷出来的东西
    int i, j, len = 1;
    ...
    for(i = len / 2; i < len; i++) {
        j = len - 1 & len - i;
        z[i] = x[i] * y[i] - (x[i] - !x[j]) * (y[i] - !y[j]) * ((cp) {
            1, 0
        }
        -w[i ^ len >> 1]) * 0.25;
    }
