...
void init()
{
	...
	npre = nend = mid;//当前长度
	npre++;
	s[npre] =s[nend] = -1;//把当前位置设置为字符集中不存在的字符
	last[0] = 1;
	last[1] = 1;
}
...
void add(char c,int q)
{
	...
	if(!nxt[cur][x])
	{
		int now = newnode();len[now] = len[cur]+2;
...
void addpre(char c){s[++nend]=c-'a';s[nend+1]=-1;add(c, 0);}
void addend(char c){s[--npre]=c-'a';s[npre-1]=-1;add(c, 1);}
char ss[LEN];
pair<int,int> a[LEN];
